import { LastPillFigure } from "@/components/last-pill-figure";
...
export function Guarantee() {
  ...
  return (
    <section id="guarantee" className="section-block px-6 lg:px-8">
      <div
        className={cn(
          "mx-auto max-w-5xl border border-cyber-titanium/12 bg-cyber-graphite px-6 py-12 sm:px-10 lg:px-14 lg:py-16",
          amino && "guarantee-panel",
        )}
      >
        <ReviewHeading eyebrow={config.eyebrow} title={config.title} />
        {amino ? <LastPillFigure /> : null}
        <div className={cn(amino && "guarantee-prose")}>
          ...
          {config.bonuses && config.bonuses.length > 0 ? (
            <ul className="mt-8 grid gap-4 sm:grid-cols-2">
              {config.bonuses.map((bonus) => (
                <li
                  key={bonus.title}
                  className="border border-cyber-titanium/12 px-5 py-4"
                >
