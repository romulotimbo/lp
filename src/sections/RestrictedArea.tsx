    ...
    <div className="relative h-full w-full shrink-0 snap-center overflow-hidden rounded-md border border-[0.5px] border-blood-red/25 bg-cyber-graphite">
      ...
      {!unlocked && (
        <div
          ...
          style={{
            backgroundImage:
              "repeating-linear-gradient(0deg, transparent, transparent 3px, rgb(var(--color-accent) / 0.1) 3px, rgb(var(--color-accent) / 0.1) 5px)",
          }}
          aria-hidden
        />
      )}
...
/** Seção opcional — só montada quando `restrictedArea` está configurado (ver product/registry.tsx). O CTA pós-unlock só aparece se o Produto também tiver `leadCapture` configurado. */
export function RestrictedArea() {
  ...
  const config = product.restrictedArea;
  const leadCaptureEnabled = product.sections.includes("lead-capture") && Boolean(product.leadCapture);
  ...
  return (
    <section className="section-block--tight relative overflow-hidden bg-cyber-darker px-4 sm:px-6 lg:px-8">
      <div
        className="pointer-events-none absolute left-1/2 top-1/2 h-[600px] w-[600px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(circle,rgb(var(--color-accent)/0.1)_0%,transparent_65%)] motion-safe:animate-pulse-slow"
        aria-hidden
      />
        ...
        <div className="overflow-hidden rounded-xl border border-blood-red/30 bg-cyber-black shadow-inner-dark motion-safe:animate-none">
          ...
          >
            ...
            <span className="text-cyber-muted">
              {unlocked
                ...
                : isScanning
                  ? `scan · ${progress}%`
                  : `${config.files.length} arquivos · locked`}
            </span>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto border-b border-blood-red/10 px-3 py-2 [scrollbar-width:none] sm:px-4 [&::-webkit-scrollbar]:hidden">
            {config.files.map((file) => (
              ...
              >
                {file.name}{" "}
                <span className="text-blood-red/50">[{unlocked ? "OPEN" : "LOCK"}]</span>
              </span>
            ))}
          </div>
          ...
          <div className="relative border-b border-blood-red/10 px-3 py-3 sm:px-4">
            ...
            <div
              ...
              aria-label={config.galleryAriaLabel}
            >
              {config.previewAssets.map((asset) => (
                <div
                  key={asset.src}
          ...
          <div className="p-3 sm:p-4">
            ...
            <button
              ...
              aria-valuemin={0}
              aria-valuemax={100}
              aria-label={config.holdAriaLabel}
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-cyber-darker">
                <motion.div className="h-full bg-blood-red shadow-neon-red" style={{ width: barWidth }} />
              </div>
            </button>
