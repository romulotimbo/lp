...
### Requirement: Review-skeptic optional modules
A Produto config SHALL be able to independently enable, disable, and order the review-skeptic modules `skepticism`, `investigation`, `test-diary`, `ugc-mosaic`, `protocol-verdict`, `synergy`, `honesty-scale`, and `safe-buy`. A `review` or `review-offer` Instância SHALL NOT list those ids. Listing them on those layouts SHALL fail the build.

#### Scenario: Review-skeptic orders narrative acts
- **WHEN** a review-skeptic Produto lists `skepticism` before `investigation` and `protocol-verdict` before `synergy`
- **THEN** the Instância SHALL render those modules in that list order

#### Scenario: Editorial review rejects skeptic modules
- **WHEN** a `review` Produto lists `"skepticism"` in `sections`
- **THEN** the build SHALL fail before producing an Instância

## MODIFIED Requirements

### Requirement: Mandatory core sections
Every sales Instância SHALL include a Hero section, a Pricing section, and a footer, regardless of optional modules. Every review Instância SHALL include a Hero section, an outbound CTA, and a footer, and SHALL NOT be required to include Pricing. Every review-offer Instância SHALL include a Hero section, an outbound CTA, and a footer, and SHALL NOT be required to include Pricing. Every review-skeptic Instância SHALL include the editorial transparency bar, a Hero section with the protocol quiz, dual catalog hops, a footer with compliance-page links, and SHALL NOT be required to include Pricing.
...
#### Scenario: Minimal review-skeptic Produto config
- **WHEN** a review-skeptic Produto config specifies no optional sections at all
- **THEN** the built Instância SHALL still render the editorial bar, Hero, protocol quiz, dual hops, and the footer, and SHALL NOT render Pricing

### Requirement: Optional section modules
A Produto config SHALL be able to independently enable, disable, and order each of the following section modules: Manifesto/Spokesperson, mechanism sections (e.g. Power Grid, Tech Mechanism), Testimonials, FAQ, lead capture, a restricted/bonus-content section, the editorial modules pain, research, official-claims, and verdict, the review conversion modules trust, highlights, ritual, compare, guarantee, and mid-cta, the review-offer modules what-is, formula, authenticity, side-effects, pros-cons, and offer, and the review-skeptic modules skepticism, investigation, test-diary, ugc-mosaic, protocol-verdict, synergy, honesty-scale, and safe-buy.
