...
A click on a review or review-offer outbound CTA (Hero, verdict, authenticity, offer, guarantee, mid-cta, footer, or sticky bar navigating to a hop/official URL) SHALL NOT fire the checkout-initiated event (`InitiateCheckout`, Google Ads checkout conversion, or equivalent). The generic checkout-click mechanism SHALL apply only to sales checkout CTAs.
...
#### Scenario: Review-offer hop click is not a checkout event
- **WHEN** a visitor clicks the outbound CTA on a review-offer Instância
- **THEN** the system SHALL navigate to the hop/official URL and SHALL NOT fire a checkout-initiated event to any configured tracking tag
