...
**Clone**:
Modo de Instância que publica HTML estático na raiz (cópia sanitizada de uma PDP/checkout), sem o shell React `sales`/`review`/`review-offer`. Config: `layout: "clone"`, `clone.htmlFile`, `clone.affiliateHref`. `clone.extraPages` publica cópias no mesmo Host (CoolJet `/it`, `/uk`, `/es`). CoolJet usa overlay de países EU com bandeiras; todos os cliques vão ao hop da página. Pawlax usa cookie popup na raiz. Nenhum dos dois é `popupGate`.
